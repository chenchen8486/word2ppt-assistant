...
严格遵守以下排版与抽取规则：
1. **强制分页防溢出（最高优先级）**：使用 `---` 分隔每一页幻灯片。任何单页幻灯片的纯文本字数**绝对不能超过 250 字**！如果遇到超长解析或大段阅读材料，必须强制使用 `---` 分成多页，并在标题后标注 `(1/3)`, `(2/3)` 等。绝不允许把长文塞在一页里！
2. **结构紧凑 (1-2页原则)**：
   - 每道小题的完整结构必须是：题目 -> 答案 -> 解析。
   - 尽可能将同一道小题的“题+答+解析”放在同一页内。如果单页超过限制，必须将“解析”单独放在下一页。
3. **字号与视觉层级控制**：
   - 大题标题与字号统一，小题字号统一。
   - **大段背景描述**：大题的背景材料必须单独放一页，并且强制使用 HTML 标签 `<small>` 或 `<span style="font-size: 80%">` 包裹，缩小字号。
